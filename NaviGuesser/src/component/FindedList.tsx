...
export default function FindedList({allStopsFinded,setPointed}: any) {
  const findedList = allStopsFinded.map((stop: any) => {
    ...
    return (
      <li key={stop.stop_id}>
        <div className="flex items-center w-full gap-2 p-2 transition-colors border border-transparent rounded-lg group hover:shadow hover:border-slate-50 hover:bg-slate-800" onClick={()=>setPointed(stop)}>
